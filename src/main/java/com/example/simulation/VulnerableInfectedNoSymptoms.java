...
public class VulnerableInfectedNoSymptoms extends State{
    private double infectingChance = 0.5;
    ...
    VulnerableInfectedNoSymptoms(Person person) {
        super(person);
                ...
                if (distance <= distanceToInfect && timeSinceClose >= minTimeToInfect) {
                    ...
                    if (rand.nextDouble() < infectingChance){
                        if(rand.nextDouble() < 0.5){
                            ...
                            System.out.println("Infected by a person without symptoms");
                        }
                        else{
    ...
    public void getHealthy(){
        if (person.getArea().getActualTime() - person.getTimeSinceInfected() > infectionDuration){
            System.out.println("A person without symptoms got healthy");
